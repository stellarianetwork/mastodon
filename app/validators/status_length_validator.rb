...
class StatusLengthValidator < ActiveModel::Validator
  ...
  def countable_text(str)
    ...
    entities = Extractor.remove_overlapping_entities(LabeledLinkParser.extract_entities_with_indices(str) + Extractor.extract_urls_with_indices(str, extract_url_without_protocol: false) + Extractor.extract_mentions_or_lists_with_indices(str))

    rewrite_entities(str, entities) do |entity|
      if entity[:label]
        'x' * [countable_length(entity[:label]), URL_PLACEHOLDER_CHARS].max
      elsif entity[:url]
        URL_PLACEHOLDER
